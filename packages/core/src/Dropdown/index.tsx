        ...
        <Menu as="div" className={"relative inline-block text-left"}>
            {customTrigger ? (
                ...
                <Menu.Button as={(props => <Button as="button" {...triggerProps} {...props} />) as unknown as "button"}>
                    {label}
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true" className="ml-2 -mr-1 h-5 w-5 text-gray-300">
                        ...
                        (child.type as any)?.displayName === "Divider" ?
                            ...
                            (
                                <Menu.Item as={React.Fragment}>
                                    {({ active }) => (
                                        <li {...itemRootProps} className={clsx("group", active && "dropdown-active", itemRootProps?.className)}>{child}</li>
                                    )}
                                </Menu.Item>
                            )
                    )}
