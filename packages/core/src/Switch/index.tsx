...
export function Switch({ id, label, labelProps, size = 'md', ...props }: { size?: 'sm' | 'md' | 'lg' } & IToggleBox) {
    ...
    const rootPseudoClassNames = [
        ...
        'peer-focus:ring-tulsa-500',
        'peer-active:ring-2',
        'peer-active:ring-offset-2',
        'peer-active:ring-offset-gray-100',
        'peer-active:ring-tulsa-500',
    ].join(' ');

    return (
        <label htmlFor={formId.current} className='flex items-center cursor-pointer relative'>
            <input type='checkbox' value='' id={formId.current} {...props} className={clsx('sr-only peer', props.className)} />
            <div
                className={clsx(
                    getSwitchSize(),
                    getSwitchKnobSize(),
                    rootPseudoClassNames,
                    'bg-gray-200 rounded-full peer peer-checked:bg-tulsa-500',
